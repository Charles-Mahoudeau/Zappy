...
bool Inventory::canAfford(const Inventory& other) const {
    return std::ranges::all_of(other._resources, [this](const auto& resource) {
        const auto& [type, required] = resource;

        return required == 0 || resourceCount(type) >= required;
    });
}
