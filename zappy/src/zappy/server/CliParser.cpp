...
T CliParser::stringToUnNumber(std::string_view value, std::string_view flagName) {
    ...
    if (!value.empty() && value.at(0) == '-') {
        throw exception::InvalidArgument(std::format(kOVERFLOW_MESSAGE, flagName, value, maxVal));
    }
    try {
        size_t pos = 0;
...
void CliParser::ensureValidArguments() const {
    ...
    for (const auto& teamName : this->_parameters.teamsName) {
        if (teamName == kNAME_KEYWORD) {
            throw exception::InvalidArgument(
                std::format("{} is a Keyword. You cannot use it as team name", kNAME_KEYWORD));
        }
