...
using CLI = zappy::server::CliParsing;
...
TEST(CliParsing, valid_full_arguments_parsed_correctly) {
...
TEST(CliParsing, unknown_flag_throws) { ASSERT_THROW(CLI::parseArguments({"-z", "42"}), InvalidArg); }

TEST(CliParsing, no_leading_dash_throws) { ASSERT_THROW(CLI::parseArguments({"p", "4242"}), InvalidArg); }

TEST(CliParsing, double_dash_flag_throws) { ASSERT_THROW(CLI::parseArguments({"--port", "4242"}), InvalidArg); }

TEST(CliParsing, only_dash_flag_throws) { ASSERT_THROW(CLI::parseArguments({"-", "4242"}), InvalidArg); }

// ─────────────────────────────────────────────
//  Parameter count
// ─────────────────────────────────────────────

TEST(CliParsing, flag_without_required_parameter_throws) { ASSERT_THROW(CLI::parseArguments({"-p"}), InvalidArg); }
...
TEST(CliParsing, non_numeric_value_throws) { ASSERT_THROW(CLI::parseArguments({"-p", "abc"}), InvalidArg); }

TEST(CliParsing, empty_value_throws) {
    // "-p" followed by "" — but "" doesn't start with '-' so it's a param
    ASSERT_THROW(CLI::parseArguments({"-p", ""}), InvalidArg);
}

TEST(CliParsing, partial_numeric_value_throws) {
    // stoull would parse "42" then fail on "abc"
    ASSERT_THROW(CLI::parseArguments({"-p", "42abc"}), InvalidArg);
}

TEST(CliParsing, negative_value_throws) {
...
TEST(CliParsing, uint16_field_above_max_throws) {
    // port is uint16_t (max 65535)
    ASSERT_THROW(CLI::parseArguments({"-p", "65536"}), InvalidArg);
}

TEST(CliParsing, uint16_field_at_max_accepted) {
